...
------------------------+-----------------------------------
            Matched (3) |     42,343      100.00      100.00
------------------------+-----------------------------------
                  Total |     42,343      100.00
*/
...
/*
. merge 1:1 nonag_id using `enterprise_2018'
(variable numind was byte, now float to accommodate using data's values)
(variable hhid was long, now double to accommodate using data's values)

    Result                      Number of obs
    -----------------------------------------
    Not matched                        39,173
        from master                    39,173  (_merge==1)
        from using                          0  (_merge==2)

    Matched                             3,170  (_merge==3)
    -----------------------------------------
*/
...
. merge 1:1 hhid numind using `employment_data'
    ...
    -----------------------------------------
    Not matched                         7,301
        from master                     7,301  (_merge==1)
        from using                          0  (_merge==2)

    Matched                            35,042  (_merge==3)
    -----------------------------------------
...
. merge n:1 hhid using "${data_2018}/ehcvm_welfare_ben2018"
(label MILIEU already defined)
(label REGION already defined)
(label ZAE already defined)
